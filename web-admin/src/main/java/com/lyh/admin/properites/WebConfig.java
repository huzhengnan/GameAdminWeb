...
public class WebConfig extends ServerProperties {
	private static final String BUNDLE_NAME = "/config/web.properties";
...
//		NOTIFY_URL = GAME_BUNDLE.getProperty("notify_url");
		...
		String channelfile = GAME_BUNDLE.getProperty("channel");
		if (!ToolUtils.isStringNull(channelfile)){
			channelfile = channelfile.toLowerCase().trim();
			String  file[] = ToolUtils.split(channelfile, "_");
			if (file.length > 0){
				WeChatConfig.loadConfig(path,file[0]);
				AlipayConfig.loadConfig(path, file[0]);
			}
